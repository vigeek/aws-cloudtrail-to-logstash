#!/bin/bash
# Wrapper/init script for cloudtrail consumer
# Russ Thompson 2016

# Standard pid location 
PID_FILE="/var/run/cloudtrail-consumer.pid"

# The name of the script
THE_PROG="cloud-trail-consumer.sh"

# The path where THE_PROG lives *EDIT THIS IF NOT DEFAULT*
THE_PATH="/opt/cloudtrail-consumer"
...
function start {
  if [ ! -z "$THE_PID" ] ; then
    ...
    screen -A -m -d -S cloudtrail-consumer bash $THE_PROG &
    sleep 3 ; get_pid
    if [ ! -z $THE_PID ] ; then
